...
function webRequest(request, response){
    ...
    } else if (request.method == "GET" && request.url.indexOf("scorelist") > -1) {
        response.writeHead(200, {"Context-Type": "text/plain"});
        var songlist = path.join(__dirname, 'scorelist.txt');
        fs.readFile(songlist, 'utf8', function (err,data) {
            if (err) {
                return console.log(err);
            }
            console.log("openned scorelist");
        ...
        if(beatmap){
            beatmap.hitObjects.map(function(dict) {
                for (var prop in dict) {
                    if(prop != 'startTime' && prop != 'position' && prop!= 'objectName') {
                        delete dict[prop];
                    }
                }
                for (var prop in dict) {
...
function receiveScore(request, response){
    if (request.method == "POST" && request.url == "/uploadscore") {
        try {
